...
from Error_Detection.detectors.evaluation.auc_metrics import AucMetric
from Error_Detection.detectors.evaluation.metrics_base import Metric
from Error_Detection.detectors.evaluation.thresholding import ThresholdingStrategy, NoThresholding
...
def generate_synthetic_data(n_samples=100):
    """Generates synthetic y_true and y_score data."""
    # Generate true labels (y_true): mostly 0s with some 1s for anomalies
    y_true = np.zeros(n_samples)
    anomaly_indices = np.random.choice(n_samples, size=int(0.1 * n_samples), replace=False)
    y_true[anomaly_indices] = 1

    # Generate predicted scores (y_score): continuous values between 0 and 1
    y_score = np.random.rand(n_samples)
    # Make scores higher for true anomalies (to simulate a working anomaly detector)
    y_score[anomaly_indices] += np.random.uniform(0.3, 0.5, size=len(anomaly_indices))
    y_score = np.clip(y_score, 0, 1)  # Ensure scores stay within [0, 1]

    return y_true, y_score

# # Generate synthetic data
# y_true, y_score = generate_synthetic_data()

# # Instantiate the RangePrecisionRangeRecallAUC class
# metric = RangePrecisionRangeRecallAUC(max_samples=50, r_alpha=0.5, p_alpha=0, cardinality="reciprocal", bias="flat")

# # Compute the AUC
# auc_score = metric.score(y_true, y_score)

# print("Synthetic data test")
# print("True Labels (y_true):", y_true)
# print("Predicted Scores (y_score):", y_score)
# print("Computed AUC-PRT:", auc_score)

...
def generate_mu_candidates(
    step: float = 0.1,
    mu_min: float = 0.1,
    mu_max: float = 0.7,
) -> List[Tuple[float, float, float, float]]:
    if step <= 0:
        raise ValueError(f"mu_step 必须 > 0，收到: {step}")

    units = int(round(1.0 / step))
    if not np.isclose(units * step, 1.0):
        raise ValueError(f"mu_step={step} 无法整除 1，请使用 0.5/0.25/0.2/0.1 等")

    min_units = int(round(mu_min / step))
    max_units = int(round(mu_max / step))
    if not np.isclose(min_units * step, mu_min):
        raise ValueError(f"mu_min={mu_min} 与 step={step} 不兼容")
    if not np.isclose(max_units * step, mu_max):
        raise ValueError(f"mu_max={mu_max} 与 step={step} 不兼容")
    if min_units > max_units:
        raise ValueError(f"mu_min={mu_min} 不能大于 mu_max={mu_max}")

    candidates = []
    for a, b, c in itertools.product(
        range(min_units, max_units + 1),
        range(min_units, max_units + 1),
        range(min_units, max_units + 1),
    ):
        d = units - a - b - c
        if d < min_units or d > max_units:
            continue
        mu_1 = round(a / units, 10)
        mu_2 = round(b / units, 10)
        mu_3 = round(c / units, 10)
        mu_4 = round(d / units, 10)
        candidates.append((float(mu_1), float(mu_2), float(mu_3), float(mu_4)))
...
def evaluate_candidate(
    ...
    mu1, mu2, mu3, mu4 = [float(x) for x in mus]
    ...
    for _ in range(repeats):
        set_mu_weights(mu1, mu2, mu3, mu4)
...
def run_search(args):
    ...
    candidates = generate_mu_candidates(
        step=args.mu_step,
        mu_min=args.mu_min,
        mu_max=args.mu_max,
    )

    print(f"MU 候选参数组数量: {len(candidates)}")
    if len(candidates) == 0:
        raise RuntimeError("没有可用的 MU 候选组合")

    all_results = []
    total = len(candidates)

    for i, mus in enumerate(candidates, start=1):
        print("=" * 90)
        print(f"[{i}/{total}] 评估 MU = {tuple(round(x, 4) for x in mus)}")
        start = time.time()
        try:
    ...
    all_path = os.path.join(args.output_dir, f"mu_search_all_{stamp}.csv")
    top_path = os.path.join(args.output_dir, f"mu_search_top_{stamp}.csv")
...
def parse_args():
    parser = argparse.ArgumentParser(description="自动搜索 MU_1~4 的最优组合")
    ...
    parser.add_argument("--data-rate", type=float, default=1, help="数据读取比例，范围(0,1]")
    ...
    parser.add_argument("--mu-min", type=float, default=0.1, help="每个 mu 的最小值")
    parser.add_argument("--mu-max", type=float, default=0.7, help="每个 mu 的最大值")
    ...
    parser.add_argument("--seed", type=int, default=42, help="随机种子（用于数据注入）")

...
Unsupervise_AD_Pool = ['Sub_IForest', 'IForest', 'LOF', 'Sub_LOF', 'Sub_PCA', 'PCA', 'HBOS', 
                        'Sub_HBOS', 'KNN', 'Sub_KNN','KMeansAD', 'KMeansAD_U', 'COPOD', 'CBLOF', 'COF', 'EIF', 'RobustPCA']
...
#                         'AnomalyTransformer', 'TimesNet', 'FITS', 'Donut', 'OFA', 'MOMENT_FT', 'M2N2', 'ALLM4TS']
...
def run_CNN(data_train, data_test, window_size=100, num_channel=[32, 32, 40], lr=0.0008, n_jobs=1):
    ...
    # if train is short, reduce window size 
    if len(data_train)<600:
        window_size=50
...
def run_Donut(data_train, data_test, win_size=100, lr=1e-4, batch_size=128):
...
def run_ALLM4TS(data_train, data_test, win_size=100, batch_size=64):
    from models.ALLM4TS import ALLM4TS
    if len(data_train)<600:
        win_size=50
    input_dim = data_test.shape[1]
    model_config = {"input_dim": input_dim, "hidden_dim": 64}  # or another hidden_dim

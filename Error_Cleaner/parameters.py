...
def set_lambda_weights(lambda_1, lambda_2, lambda_3, normalize=False):
    """设置高层奖励权重 LAMBDA_1~3。"""
    global LAMBDA_1, LAMBDA_2, LAMBDA_3

    weights = np.array([lambda_1, lambda_2, lambda_3], dtype=np.float64)
    if np.any(weights < 0):
        raise ValueError(f"lambda 权重必须非负，收到: {weights.tolist()}")

    total = float(weights.sum())
    if total <= 0:
        raise ValueError(f"lambda 权重和必须 > 0，收到: {total}")

    if normalize:
        weights = weights / total

    LAMBDA_1, LAMBDA_2, LAMBDA_3 = [float(x) for x in weights]


def set_mu_weights(mu_1, mu_2, mu_3, mu_4, normalize=False):
    """设置低层奖励权重 MU_1~4。"""
    global MU_1, MU_2, MU_3, MU_4

    weights = np.array([mu_1, mu_2, mu_3, mu_4], dtype=np.float64)
    if np.any(weights < 0):
        raise ValueError(f"mu 权重必须非负，收到: {weights.tolist()}")

    total = float(weights.sum())
    if total <= 0:
        raise ValueError(f"mu 权重和必须 > 0，收到: {total}")

    if normalize:
        weights = weights / total

    MU_1, MU_2, MU_3, MU_4 = [float(x) for x in weights]
...
class RLCleanEnvironment:
    def __init__(self, dirty_data, label, task_type, detector, constraints, concentrated=True, max_steps=20, train_ratio=0.7):
        ...
        self.latest_proxy_perf_change = 0.0
        self.latest_proxy_train_time = 0.0
        self.k_history = []
        self.best_k = 2
        # 初始化缓存检测结果
    ...
    def step_low(self, macro_action, low_action):
        ...
        # low层执行清洗算子后，立即调用proxy模型获得最新下游性能变化
    ...
    def step_high(self, macro_action, low_reward, is_final=False):
        ...
        perf_change = getattr(self, 'latest_proxy_perf_change', 0.0)
        cur_time = getattr(self, 'latest_proxy_train_time', 0.0)
        test_perf_change = perf_change
        ...
        info = {
            ...
            'low_rl_reward': reward_components['perf_component'],  # 来自低层奖励分量
            ...
            'k_stability_reward': 0.0  # 当前高层奖励中不单独计入k稳定性
        }
...
# 主函数
# ==============================
def main(dirty_data, label=None, task_type='forecast', detector=None, constraints=None, concentrated=True, max_steps=20, n_episodes=10, return_details=False):
    '''
    ...
    perf_improvement = final_test_perf - env.initial_final_perf
    details = {
        'final_test_perf': float(final_test_perf),
        'perf_improvement': float(perf_improvement),
        'total_time_cost': float(total_time_cost),
        'best_k': float(env.best_k),
    }

    # 返回清洗后的数据、时间成本以及训练好的策略模型
    if return_details:
        return env.cur_train, total_time_cost, (macro_agent, low_agent), details
...
if __name__ == '__main__':
    ...
    model_save_path = f"/home/yyy/TSC/TSClean/AutoClean/Error_Cleaner/saved_models/original/{task_type}_{dataset_name}_original_5.pkl"

...
dump(config_headers, '/home/yyy/TSC/TSClean/AutoClean/Error_Detection/MetaOD/trained_models/model_list.joblib')
...
for j in range(len(csv_file_list)):
    ...
    X = pd.read_csv(os.path.join("/home/yyy/TSC/TSClean/AutoClean/Error_Detection/Train_Datasets", csv_file.split('_')[0],csv_file + '_val.csv')) #######val 和 train
    X = X.iloc[:, 1:-1]
    print(X.shape)
...
# The DataFrame was created with the dataset names as the index (via
# index=csv_file_list). Instead of calling set_index on a non-existent
# column, name the index 'Dataset' so later code that expects that label
# works correctly, then transpose.
meta_df.index.name = 'Dataset'

df_transposed = meta_df

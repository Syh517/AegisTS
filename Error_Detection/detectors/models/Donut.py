...
from Error_Detection.detectors.utils.torch_utility import EarlyStoppingTorch, get_gpu
...
class Donut():
    ...
    def decision_function(self, data):
        if len(data) < self.win_size:
            print(f"[WARNING] Data length ({len(data)}) is shorter than win_size ({self.win_size}). Returning zeros.")
            self.__anomaly_score = np.zeros(len(data))
            return self.__anomaly_score
        ...
        with torch.no_grad():
            for idx, (x, _) in loop:
                ...
                x_dec_mean = x_dec_mean.view(self.mc_samples, B, T, D)
                x_dec_std = x_dec_std.view(self.mc_samples, B, T, D)
                nll_output = torch.sum(F.gaussian_nll_loss(
                    x_dec_mean[:, :, -1, :],
                    x[:, -1, :].unsqueeze(0),
                    x_dec_std[:, :, -1, :]**2,
                    reduction='none'
        ...
        if not scores:
            print("[ERROR] No scores were computed. This might indicate the test_loader was empty.")
            self.__anomaly_score = np.zeros(len(data))
            return self.__anomaly_score

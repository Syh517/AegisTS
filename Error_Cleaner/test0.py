...
ALPHA = 0.1 # k
MU_1, MU_2, MU_3 = 0.6, 0.2, 0.2 
LOW_DOWNSTREAM_BETA = 0.4  # low层更新时混入high层的下游性能反馈权重
...
def train_hrl_agents(env, macro_agent, low_agent, n_episodes):
    ...
    for episode in range(n_episodes):
        ...
        while not done:
            ...
            # 将一部分high层下游性能反馈回传给low层，缓解局部最优与下游目标不一致
            test_change_for_low = np.clip(info.get('test_perf_change', 0.0), -1.0, 1.0)
            blended_low_reward = reward_l + LOW_DOWNSTREAM_BETA * test_change_for_low
            low_trajectory = [(state_l, low_action, blended_low_reward, next_state_l, (low_done or high_done))]

...
def _format_axes(ax, n_points):
    ...
    xticks = sorted(set(list(range(0, n_points + 1, 10)) + [21]))
    ax.set_xticks(xticks)
    ax.set_xticklabels(["20" if t == 21 else str(t) for t in xticks])
    ax.set_ylim(0, 10)
    ...
    for tick, label in zip(ax.get_xticks(), ax.get_xticklabels()):
        if abs(tick - 21) < 1e-9:
            ...
            offset = transforms.ScaledTranslation(12 / 72, 0, ax.figure.dpi_scale_trans)
...
def plot_all_quality_problems(x, HULL, MULL, n_points, output_dir, fig, ax):
    ...
    # 2. 添加异常值
    single_index = 11
    single_magnitude = 3.0 * float(HULL.std())
    drift_start = 12
    ...
    # 4. 添加重复值
    HULL_dirty.iloc[21] = HULL_dirty.iloc[20]
    MULL_dirty.iloc[21] = MULL_dirty.iloc[20]
    ...
    # 标记异常值点
    ax.scatter([single_index], [HULL_dirty.iloc[single_index]], color="red", marker="o", s=150, zorder=6, label="Outliers")
    ax.scatter(x[drift_start:drift_start + drift_length], [MULL_dirty.iloc[i] for i in range(drift_start, drift_start + drift_length)], color="red", marker="o", s=150, zorder=6)

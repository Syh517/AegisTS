...
class RowConstraintMiner:
    """
    极致优化版行约束挖掘器：
    1. 引入相关性预筛选 (Corr Screening)，提升挖掘速度。
    2. 引入多重共线性检查，确保公式稀疏且稳定。
    3. 优化精简逻辑，遵循奥卡姆剃刀原则。
    """
    ...
    def __init__(
        ...
        df=None,
        degree: int = 2,              
        max_features: int = 2,        # 默认 2 个变量，行约束通常简单
        coverage: float = 0.95,       # 提高覆盖率要求
        max_residual_std: float = 0.3, # 降低残差容忍度，追求精确
        min_r2: float = 0.8,         # 只有高拟合度才视为“约束”
    ):
    ...
    def _preprocess_df(self, df):
        ...
        return df.select_dtypes(include=[np.number]).dropna().reset_index(drop=True)
    ...
    def row_miner(self) -> List[Dict[str, Any]]:
        ...
        for target in columns:
            ...
            # --- 优化点 1: 特征预筛选 ---
            # 只选择与 target 相关性较强的列，剔除无关噪音
            potential = corr_matrix[target].drop([target, "timestamp"], errors='ignore')
            # 仅保留相关性 > 0.3 的前 10 个候选特征
            candidates = potential[potential > 0.3].sort_values(ascending=False).head(10).index.tolist()

            for n in range(1, min(self.max_features, len(candidates)) + 1):
                for feat_combo in combinations(candidates, n):
                    
                    # --- 优化点 2: 多重共线性检查 ---
                    # 如果自变量之间本身就高度线性相关，挖掘出的公式将是不稳定的
                    if n > 1:
                        sub_corr = df[list(feat_combo)].corr().abs()
                        # 若自变量间相关性 > 0.98，跳过
                        if (sub_corr.values[np.triu_indices_from(sub_corr, k=1)] > 0.98).any():
                            continue

                    X_raw = df[list(feat_combo)].values
                    y = df[target].values
                    
                    try:
                        poly = PolynomialFeatures(degree=self.degree, include_bias=False)
                        X_poly = poly.fit_transform(X_raw)
                        feature_names = poly.get_feature_names_out(list(feat_combo))
                        ...
                        # --- 优化点 3: 严格阈值控制 ---
                        if r2 < self.min_r2 or res_std > self.max_residual_std:
                            continue
                        ...
                        lower, upper = np.quantile(residuals, [tail, 1-tail])

                        # 创建公式字符串
                        model_terms = coef_dict
                        feature_part = " + ".join([f"{coef:.6f}*{feat}" for feat, coef in model_terms.items()])
                        formula = f"{target} = {feature_part} + {model.intercept_:.6f} | residual ∈ [{lower:.6f}, {upper:.6f}]"
                        ...
                        raw_constraints.append({
                            ...
                            "upper": upper,
                            "R2": float(r2),
                            "loss_mse": float(mean_squared_error(y, pred)),
                            "residual_std": float(res_std)
                        })
                    except: continue

        self.constraints = self._refine_constraints(raw_constraints)
    ...
    def _refine_constraints(self, constraints: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for target, group in df_c.groupby("target"):
            # 策略：优先选择特征最少且 R2 优秀的模型
            group = group.sort_values(["R2"], ascending=False)
            best_r2 = group["R2"].iloc[0]
            
            # 允许 0.5% 的 R2 性能损失来换取更简单的模型
            simpler_models = group[group["R2"] >= best_r2 - 0.005].copy()
            simpler_models["n_feat"] = simpler_models["features"].apply(len)
            best_row = simpler_models.loc[simpler_models["n_feat"].idxmin()]
            
            res_item = best_row.to_dict()
            # 剔除噪音权重 (绝对值相对于最大系数小于 0.01% 的项)
            max_c = max([abs(v) for v in res_item["model_terms"].values()] + [1e-7])
            res_item["model_terms"] = {k: v for k, v in res_item["model_terms"].items() if abs(v) > max_c * 0.0001}
            
            expr = self._format_formula(res_item["model_terms"], res_item["intercept"])
            res_item["formula"] = f"{target} = {expr} | residual ∈ [{res_item['lower']:.4f}, {res_item['upper']:.4f}]"
            refined_results.append(res_item)
            
        return sorted(refined_results, key=lambda x: -x["R2"])

    def _format_formula(self, terms, intercept) -> str:
        parts = []
        for name in sorted(terms.keys(), key=self._natural_sort_key):
            coef = terms[name]
            if abs(coef) < 1e-7: continue
            parts.append(("+ " if coef >= 0 else "- ", abs(coef), name))
        
        res = f"{intercept:.4f}" if abs(intercept) > 1e-7 else ""
        for i, (sign_str, val, name) in enumerate(parts):
            if res == "":
                res = f"{'-' if sign_str.strip() == '-' else ''}{val:.4f}*{name}"
            else:
                res += f" {sign_str}{val:.4f}*{name}"
        return res or "0.0000"

    def _natural_sort_key(self, s: str):
        return [int(t) if t.isdigit() else t.lower() for t in re.split(r"(\d+)", s)]

    def _get_constraint_key(self, target, coef_dict, intercept):
        rounded_coef = tuple(sorted((k, round(v, 6)) for k, v in coef_dict.items() if abs(v) >= 1e-7))
        return (target, rounded_coef, round(intercept, 6))

    def mine_3d(self, data_3d: np.ndarray) -> List[Dict[str, Any]]:
        arr = np.asarray(data_3d)
        if arr.ndim == 2: arr = arr.reshape(1, arr.shape[0], arr.shape[1])
        df_combined = pd.DataFrame(arr.reshape(-1, arr.shape[2]))
        # 自动重命名列
        df_combined.columns = ["timestamp"] + [f"col_{j}" for j in range(1, df_combined.shape[1])]
        
        miner = RowConstraintMiner(df_combined, **self._get_params())
        return miner.row_miner()
    ...
    def mine_3d_per_sample(self, data_3d: np.ndarray) -> List[List[Dict[str, Any]]]:
        ...
        results = []
        for sample in arr:
            miner = RowConstraintMiner(sample, **self._get_params())
            results.append(miner.row_miner())
        return results

    def _get_params(self):
        return {
            "degree": self.degree, "max_features": self.max_features,
            "coverage": self.coverage, "max_residual_std": self.max_residual_std, "min_r2": self.min_r2
        }

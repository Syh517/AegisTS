...
from Error_Detection.detectors.utils.torch_utility import EarlyStoppingTorch, PositionalEmbedding, TokenEmbedding, TemporalEmbedding, get_gpu, TimeFeatureEmbedding, DataEmbedding, adjust_learning_rate
from Error_Detection.detectors.utils.dataset import ReconstructDataset    
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.gpt2 = GPT2Model.from_pretrained(
                    ...
                    cache_dir="/Volumes/LaCie/mTSB/TSB_AD/models/hf_cache",
...
class OFA():
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            with torch.no_grad():
                for i, (batch_x, _) in loop:
                    ...
                    loop.set_postfix(valid_loss=np.mean(total_loss))
    ...
    def decision_function(self, data):
        ...
        attens_energy = []
        y_hats = []
        self.anomaly_criterion = nn.MSELoss(reduction='none').to(self.device)  # move to GPU
        ...
        # Concatenate once at the end (faster)
        attens_energy = torch.cat(attens_energy).cpu().numpy()
        scores = np.array(attens_energy)

        y_hats = torch.cat(y_hats).cpu().numpy()

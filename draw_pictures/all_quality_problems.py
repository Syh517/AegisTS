...
def _format_axes(ax, n_points):
    ...
    major_xticks = list(range(0, n_points + 1, 10))
    bottom_only_xticks = [7, 8]

    ax.set_xticks(major_xticks)
    ax.set_xticklabels([str(t) for t in major_xticks])
    ax.set_xticks(bottom_only_xticks, minor=True)
    ax.set_xticklabels(["7" if t == 8 else str(t) for t in bottom_only_xticks], minor=True)
    ax.set_ylim(0, 10)
    ax.tick_params(axis='both', which='major', direction='in', top=True, right=True, pad=20, width=2.5, length=8)
    ax.tick_params(axis='x', which='minor', direction='in', top=False, bottom=True, labeltop=False, labelbottom=True, pad=20, width=2.5, length=8)
    ...
    if len(minor_labels) >= 2:
        label_8 = minor_labels[1]
        label_8.set_color("red")
        label_8.set_fontweight("bold")
        offset = transforms.ScaledTranslation(1 / 72, 0, ax.figure.dpi_scale_trans)
        label_8.set_transform(label_8.get_transform() + offset)
...
def plot_all_quality_problems(x, HULL, MULL, n_points, output_dir, fig, ax):
    ...
    """
    # 1. 添加缺失值
    missing_idx_HULL = [22,23,24,25,26]
    missing_idx_MULL = []
    ...
    # 2. 添加异常值
    single_index_1 = 14
    single_magnitude_1 = 5.0 * float(HULL.std())
    single_index_2 = 17
    single_magnitude_2 = 5.0 * float(MULL.std())
    HULL_dirty = inject_single_point(HULL_dirty, single_index_1, single_magnitude_1)
    MULL_dirty = inject_single_point(MULL_dirty, single_index_2, single_magnitude_2)
    ...
    # 4. 添加重复值
    HULL_dirty.iloc[8] = HULL_dirty.iloc[7]
    MULL_dirty.iloc[8] = MULL_dirty.iloc[7]
    ...
    MULL_viol_idx = MULL_viol_idx[(MULL_viol_idx != 9) & (MULL_viol_idx != 22) & (MULL_viol_idx != 34)]


    # 绘制原始干净数据
    ax.plot(x, HULL, color="#D0D0D0", linewidth=5, label="Clean HULL", linestyle="--", zorder=1)
    ax.plot(x, MULL, color="#D0D0D0", linewidth=5, label="Clean MULL", linestyle=":", zorder=1)

    # 绘制带有所有问题的数据
    ax.plot(x, HULL_dirty, color="#1f77b4", linewidth=5, label="Dirty HULL", zorder=2)
    ax.plot(x, MULL_dirty, color="#0C7779", linewidth=5, label="Dirty MULL", zorder=2)


    # 标记异常值点
    ax.scatter([single_index_1], [HULL_dirty.iloc[single_index_1]], color="red", marker="o", s=150, zorder=6, label="Outliers")
    ax.scatter([single_index_2], [MULL_dirty.iloc[single_index_2]], color="red", marker="o", s=150, zorder=6)


    # 标记约束违规点
    if len(HULL_viol_idx) > 0:
        ax.scatter(HULL_viol_idx, HULL_dirty.iloc[HULL_viol_idx], color="#F25912", marker="*", s=350, zorder=5, label="Constraint Violations")
    if len(MULL_viol_idx) > 0:
        ax.scatter(MULL_viol_idx, MULL_dirty.iloc[MULL_viol_idx], color="#F25912", marker="*", s=350, zorder=5)

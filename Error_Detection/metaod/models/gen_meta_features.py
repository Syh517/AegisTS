...
def generate_meta_features(X):
    ...
    if n_components >= 1:  # 至少能做 PCA
        pca_transformer = sklearn_PCA(n_components=n_components)
        X_transform = pca_transformer.fit_transform(X)

        # first pc
        pca_fpc = list_process(
            X_transform[0, :],
            r_min=False, r_max=False, r_mean=False,
            r_std=True, r_skew=True, r_kurtosis=True
        )
        meta_vec.extend(pca_fpc)
        meta_vec_names.extend(
    ...
    # ---- PCA 模型部分 ---- #
    if n_components >= 1:
        clf = sklearn_PCA(n_components=n_components)
        clf.fit(X)

        # 填充到 3 维，保持 meta-feature 维度一致
        expl_var = list(clf.explained_variance_ratio_)
        sing_val = list(clf.singular_values_)

        # 如果 n_components < 3，就补零
        while len(expl_var) < 3:
            expl_var.append(0.0)
        while len(sing_val) < 3:
            sing_val.append(0.0)

        meta_vec.extend(expl_var)
        meta_vec.extend(sing_val)
    else:
        meta_vec.extend([0.0, 0.0, 0.0])  # explained variance ratio
        meta_vec.extend([0.0, 0.0, 0.0])  # singular values

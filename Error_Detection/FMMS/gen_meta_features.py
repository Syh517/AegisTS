...
def flatten_diagonally(x, diags=None):
    # Handle edge case where x is 1-dimensional or empty
    if x.ndim < 2:
        return np.array([])
...
def list_process(x, r_min=True, r_max=True, r_mean=True, r_std=True,
    ...
    # Handle empty arrays
    if x.size == 0:
        default_value = 0  # Using 0 as default for empty arrays
        if r_min:
...
def generate_meta_features(X):
    ...
    # Handle non-finite values before calling check_array.
    # Some datasets can become all-NaN or contain inf values after cleaning steps,
    # which breaks PCA-based meta-feature extraction downstream.
    X = np.asarray(X, dtype=np.float64)
    X = np.atleast_2d(X)

    finite_mask = np.isfinite(X)
    if np.any(finite_mask):
        fill_value = float(np.mean(X[finite_mask]))
    else:
        fill_value = 0.0

    X = np.where(finite_mask, X, fill_value)
    ...
    quant_coeff_disp = (q75 - q25) / (q75 + q25 + 1e-8)
    ...
    n_components = min(3, n_features)  # Set to 3 or the number of features, whichever is smaller
    try:
        X_pca = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)
        pca_transformer = sklearn_PCA(n_components=n_components, svd_solver='randomized', random_state=42)
        X_transform = pca_transformer.fit_transform(X_pca)
        pca_fpc = list_process(X_transform[0, :], r_min=False, r_max=False,
                               r_mean=False,
                               r_std=True, r_skew=True, r_kurtosis=True)
    except Exception:
        pca_fpc = [0.0, 0.0, 0.0][:n_components]
        while len(pca_fpc) < 3:
            pca_fpc.append(0.0)
    ...
    try:
        ...
        # 如果HBOS拟合失败，使用默认值填充
        n_features_for_hbos = X.shape[1] if len(X.shape) > 1 else 1
        default_values = [0.5] * n_features_for_hbos  # 使用0.5作为默认值
        meta_vec.extend(list_process(default_values))  # HBOS_mean
        meta_vec.extend(list_process(default_values))  # HBOS_max
        meta_vec.extend(list_process(default_values))  # HBOS_min

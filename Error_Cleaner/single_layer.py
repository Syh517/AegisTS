...
from Error_Cleaner.strategy import (
    save_single_layer_agent,
    load_single_layer_agent,
    apply_single_layer_model
)

# ==============================
# 常量
# ==============================
LAMBDA_1, LAMBDA_2, LAMBDA_3, LAMBDA_4 = 0.55, 0.05, 0.15, 0.25  # LAMBDA_1 * R_Perf_N - LAMBDA_2 * R_Cost_N + LAMBDA_3 * R_Issue_N + LAMBDA_4 * low_reward_N
...
def get_all_possible_actions():
    actions = []
    for macro_action, low_actions in LOW_ACTIONS_MAP.items():
        for low_action in low_actions:
            actions.append((macro_action, low_action))
    if ('FINISH', 'NONE') not in actions:
        actions.append(('FINISH', 'NONE'))
    return actions
...
def _has_consecutive_repeated_actions_with_minimal_gain(env, threshold=0.0003, consecutive_count=2):
    ...
    Parameters:
        ...
        threshold: 性能增益阈值，低于此值视为无增益
        consecutive_count: 需要检查的连续动作次数，默认为2
        
    Returns:
        bool: 是否存在连续重复动作且性能增益微小
    ...
    # 如果最近consecutive_count次动作相同且性能增益都小于阈值
    if (len(recent_actions) == consecutive_count and 
        ...
        # 检查这consecutive_count次连续重复动作的性能变化量是否都很小
...
class RLCleanEnvironment:
    def __init__(self, dirty_data, label, task_type, detector, constraints, concentrated=True, max_steps=20, train_ratio=0.7):
        ...
        self.total_cost = 0.0
        self.proxy_train_time_last = 0.0
        self.k_history = []
        self.best_k = 2
        # 初始化缓存检测结果
    ...
    def _create_proxy_model(self):
        if self.task_type == 'classification':
            ...
            X = np.nan_to_num(X).swapaxes(1, 2)  # 转置为 (n_samples, n_channels, n_timepoints)
    ...
    def _create_final_model(self):
        if self.task_type == 'classification':
            ...
            # 先计算基于实际数据的best_k值
            X = self.cur_train[:, :, self.numeric_cols].copy()
            X = np.nan_to_num(X).swapaxes(1, 2)
            best_k, _ = auto_select_k(X, (K_MIN, K_MAX))
            self.best_k = best_k  # 更新best_k
    ...
    def step_high(self, macro_action, low_reward, is_final=False):
        ...
        self.prev_test_perf = self.last_test_perf

        if is_final or self.step_count > self.max_steps:
            final_perf, _ = train_and_evaluate(self.task_type, 'final', self.final_model, self)
            

            R_H = LAMBDA_GRAD * (final_perf - self.initial_final_perf)
            # 提供更详细的下游任务性能信息
            info = {
                'final_perf': final_perf,
                'initial_final_perf': self.initial_final_perf,
                'perf_improvement': final_perf - self.initial_final_perf,
                'best_k': self.best_k,
            }
        ...
        perf_change, cur_time = train_and_evaluate(self.task_type, 'proxy', self.proxy_model, self)
        test_perf_change = perf_change  # 直接使用train_and_evaluate返回的性能变化值
        ...
        R_Perf = test_perf_change
        
        time_inc = cur_time - self.proxy_train_time_last
        R_Cost = max(0, time_inc)
              
        # 添加指标改进的奖励
        R_Issue = missing_improvement + outlier_improvement + violation_improvement

        # 归一化高层奖励分量，避免量纲影响
        R_Perf_N = np.clip(R_Perf, -1.0, 1.0)
        ...
        # 更新动作效果记录
        self.recent_action_effects[macro_action] = test_perf_change
        ...
        self.performance_history.append(test_perf_change)
        ...
        reward_components = {
            'perf_component': LAMBDA_1 * R_Perf_N,
        ...
        # 根据性能变化更新last_test_perf
        self.last_test_perf = self.prev_test_perf + perf_change
        
        self.proxy_train_time_last = cur_time
        self.total_cost += max(0, time_inc)

        info = {
            'grad_reward': reward_components['perf_component'],
            ...
            'best_k': self.best_k,
            'test_perf_change': test_perf_change,
            'test_perf': self.last_test_perf,
...
def _normalize_action(action):
    ...
    if isinstance(action, tuple):
        return tuple(_normalize_action(a) for a in action)
    return action

class SingleLayerAgent(HRLAgent):
    def __init__(self, all_actions, state_dim, **kwargs):
        super().__init__(all_actions, state_dim, **kwargs)
        self.all_actions = [_normalize_action(action) for action in all_actions]
    ...
    def choose(self, state):
        ...
        if key not in self.q:
            self.q[key] = np.full(len(self.all_actions), 50.0)
        if self.rng.random() < self.epsilon():
            return _normalize_action(self.rng.choice(self.all_actions))
        return _normalize_action(self.all_actions[int(np.argmax(self.q[key]))])
    ...
    def update(self, trajectory):
        for s, a, r, s_next, done in reversed(trajectory):
            ...
            if k not in self.q:
                self.q[k] = np.full(len(self.all_actions), 50.0)
            next_max_q = 0.0
            if k_next in self.q and not done:
                next_max_q = np.max(self.q[k_next])
            target = r + self.gamma * next_max_q
            idx = self.action_to_idx.get(a)
            if idx is None:
                logging.warning(f"Action '{a}' not in action space {self.all_actions}. Skipping Q-update.")
                continue
...
def macro_agent_choose_with_rates(macro_agent, state, current_issue_rates, env=None):
    ...
    # 检查是否需要强制更换工具 - 当同一个(high_action, low_action)组合连续执行且没有明显性能提升时
    if env:
        ...
        if has_repeated:
            ...
            if explorable:
                print(f"强制更换工具: 连续执行 {repeated_action} 两次且无性能提升，强制选择其他工具")
...
def low_agent_choose_override(low_agent, state, macro_action, env=None):
    ...
    # 检查是否需要强制更换工具 - 当连续三次执行相同的low action且没有性能提升时
    if env and len(env.action_history) >= 3:
        # 检查最近三次动作是否都是相同的(macro_action, low_action)组合且性能提升很小
        last_three_actions = env.action_history[-3:]
        last_three_performances = env.performance_history[-3:]
        
        # 检查是否最近三次执行的都是相同的macro_action和low_action，且性能提升都很小
        same_actions = all(
        ...
        if same_actions and small_improvements:
            ...
            if explorable:
                print(f"强制更换低级工具: 连续执行 {macro_action}-{current_low_action} 三次且无性能提升，强制选择其他工具")
    ...
    if env and hasattr(env, 'action_history') and hasattr(env, 'performance_history') and len(env.action_history) >= 1:
        ...
        for i, action in enumerate(low_actions):
            if action in action_effects:
                ...
                if abs(avg_effect) <= 0.0005:  # 几乎没有改进的动作
...
def single_agent_choose_with_repeat_check(agent, state, env=None):
    agent.steps += 1
    key = agent._key(state)
    if key not in agent.q:
        agent.q[key] = np.zeros(len(agent.all_actions))

    if env is not None:
        rates, _, _, _, _, _ = env._get_issue_rates()
        if rates[ISSUE_MAP['MISSING']] > 0.0:
            missing_actions = [a for a in agent.all_actions if a[0] == 'MISSING']
            if missing_actions:
                return _normalize_action(agent.rng.choice(missing_actions))

    if env:
        has_repeated, repeated_action = _has_consecutive_repeated_actions_with_minimal_gain(env)
        if has_repeated:
            explorable = [a for a in agent.all_actions if a != repeated_action and a[0] != 'FINISH']
            if explorable:
                print(f"强制更换工具: 连续执行 {repeated_action} 且无性能提升，强制选择其他工具")
                return _normalize_action(agent.rng.choice(explorable))

    if agent.rng.random() < agent.epsilon():
        explorable = [a for a in agent.all_actions if a[0] != 'FINISH']
        return _normalize_action(agent.rng.choice(explorable))

    q_values = agent.q[key].copy()

    if env and hasattr(env, 'action_history'):
        action_counts = {}
        for action in env.action_history:
            action_counts[action] = action_counts.get(action, 0) + 1
        for i, action in enumerate(agent.all_actions):
            count = action_counts.get(action, 0)
            if count > 2:
                q_values[i] -= 0.1 * count

    if ('FINISH', 'NONE') in agent.all_actions and agent.steps < 5:
        q_values[agent.all_actions.index(('FINISH', 'NONE'))] = -np.inf

    return _normalize_action(agent.all_actions[int(np.argmax(q_values))])
    ...
    for episode in range(n_episodes):
        ...
        while not done:
            ...
            if macro_action == 'FINISH':
                ...
                # 输出最终任务的详细性能信息
                final_perf = info.get('final_perf', 0.0)
                initial_perf = info.get('initial_final_perf', 0.0)
                perf_improvement = info.get('perf_improvement', 0.0)
                print("\n")
                print(f"Episode {episode+1}/{n_episodes} - FINAL TASK COMPLETED")
                print(f"  Final Performance: {final_perf:.4f}")
                print(f"  Initial Performance: {initial_perf:.4f}")
                print(f"  Improvement: {perf_improvement:+.4f}")
                print("\n")
            ...
            print(f"Episode {episode+1}/{n_episodes} - Step: {env.step_count:2d} "
                  ...
                  f"- TestPerf: {info.get('test_perf', 0):.4f} "
                  f"- ΔTest: {test_change:+.4f} "
                  f"- Penalty: {extra_penalty:+.4f} "
                  f"- best_k: {info.get('best_k', env.best_k)}")
            print(f"  [详细Reward分解] Gradient:{info.get('grad_reward', 0):+.3f}, "
                  f"Issue:{info.get('issue_reward', 0):+.3f}, "
...
def train_single_layer_agent(env, agent, n_episodes):
    reward_history = []
    for episode in range(n_episodes):
        state = env.reset()
        done = False
        ...
        while not done:
            action = single_agent_choose_with_repeat_check(agent, state, env)
            macro_action, low_action = action
            ...
            if macro_action == 'FINISH':
                env.step_count += 1
                next_state, reward_h, done, info = env.step_high(macro_action, 0.0, is_final=True)
                trajectory.append((state, action, reward_h, next_state, done))
                episode_reward += reward_h
                print("\n")
                print(f"Episode {episode+1}/{n_episodes} - FINAL TASK COMPLETED")
                print(f"  Final Performance: {info.get('final_perf', 0.0):.4f}")
                print(f"  Initial Performance: {info.get('initial_final_perf', 0.0):.4f}")
                print(f"  Improvement: {info.get('perf_improvement', 0.0):+.4f}")
                print("\n")
                break

            _, reward_l, low_done = env.step_low(macro_action, low_action)
            next_state, reward_h, high_done, info = env.step_high(macro_action, reward_l)
            ...
            print("\n")
            print(f"Episode {episode+1}/{n_episodes} - Step: {env.step_count:2d} "
                  f"- Action: ({macro_action}, {low_action}) "
                  f"- Reward: {reward_h:+6.3f} "
                  f"- TestPerf: {info.get('test_perf', 0):.4f} "
                  f"- ΔTest: {test_change:+.4f} "
                  f"- Penalty: {extra_penalty:+.4f} "
...
def main(dirty_data, label=None, task_type='forecast', detector=None, constraints=None, concentrated=True, max_steps=20, n_episodes=10):
    ...
    # 然后获取状态维度
    state_dim = env._state_high().shape[0]
    all_actions = get_all_possible_actions()
    agent = SingleLayerAgent(all_actions=all_actions, state_dim=state_dim, lr=0.15, gamma=0.9)

    print("\n--- 开始单层强化学习训练")
    _ = train_single_layer_agent(env, agent, n_episodes=n_episodes)
    print("--- 训练完成 ---")
    ...
    return env.cur_train, total_time_cost, agent
...
if __name__ == '__main__':
    ...
    task_type = 'forecast'
    model_save_path = f"/home/yyy/TSC/TSClean/AutoClean/Error_Cleaner/saved_models/single_layer/{task_type}_{dataset_name}_single_layer_5.pkl"
    ...
    save_single_layer_agent(trained_agents, model_save_path)


    # # 调用模型清洗
    # repaired_data, time_cost = apply_single_layer_model(

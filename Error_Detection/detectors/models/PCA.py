...
from Error_Detection.detectors.models.feature import Window
from Error_Detection.detectors.models.base import BaseDetector
# 修复导入路径错误
from Error_Detection.detectors.utils.utility import check_parameter
from Error_Detection.detectors.utils.utility import standardizer    
...
class PCA(BaseDetector):
    ...
    def __init__(self, slidingWindow=20, sub = True, n_components=None, n_selected_components=None,
    ...
    def fit(self, X, y=None):
        ...
        if self.zero_pruning:
            ...
            # Check if all features were removed by zero pruning
            if X.shape[1] == 0:
                # If all features were pruned, we can't run PCA
                # Return a dummy model with constant scores
                self.all_features_pruned = True
                self.decision_scores_ = np.zeros(n_samples)
                self._process_decision_scores()
                return self

        self.all_features_pruned = False
    ...
    def decision_function(self, X):
        ...
        # Check if the model was trained with zero features (edge case handling)
        if hasattr(self, 'all_features_pruned') and self.all_features_pruned:
            # If all features were pruned during fit, return zeros for all samples
            return np.zeros(n_samples)
        
        # For the normal case, check if components exist (i.e., PCA was run)

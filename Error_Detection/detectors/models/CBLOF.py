...
class CBLOF(BaseDetector):
    ...
    def _set_small_large_clusters(self, n_samples):
        ...
        if len(intersection) > 0:
            ...
            # If no valid separation can be formed, use a simple heuristic:
            # Take at least 2 clusters as large if available, otherwise take the largest cluster
            if self.n_clusters_ > 1:
                self._clustering_threshold = min(2, self.n_clusters_-1)  # At least 1 large cluster
            else:
                self._clustering_threshold = 1  # Only one cluster case

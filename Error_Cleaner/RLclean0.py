...
# 常量
# ==============================
LAMBDA_1, LAMBDA_2, LAMBDA_3 = 20.0, 0.01, 10.0
LAMBDA_GRAD = 50.0
ALPHA_RL = 5
MU_1, MU_2, MU_3 = 0.5, 0.001, 5.0
EPSILON = 0.1
...
def _has_consecutive_repeated_actions_with_minimal_gain(env, threshold=0.0005):
    """
    检查是否有连续重复的动作且性能增益微小
    ...
    # 检查最近的动作历史
    recent_actions = env.action_history[-2:]  # 最近2次动作
    recent_performances = env.performance_history[-2:] if len(env.performance_history) >= 2 else []
    
    # 如果最近两次动作相同且性能增益小于阈值
    if (len(recent_actions) == 2 and recent_actions[0] == recent_actions[1] and 
        len(recent_performances) == 2):
        # 检查这两个连续重复动作的性能变化量是否都很小
        if abs(recent_performances[0]) <= threshold and abs(recent_performances[1]) <= threshold:
...
def auto_select_k(X, k_range=(2, 10)):
    ...
    X = np.nan_to_num(X, nan=0.0, copy=True)  # 确保没有 NaN 值
...
def train_and_evaluate(task_type, model_type, model_instance, env):
    ...
    if model_type == 'proxy':
        ...
        try:
            if task_type == 'classification':
                ...
                X = np.nan_to_num(X_train_temp).swapaxes(1, 2)
                y = y_train_temp
                ...
                X_all = np.nan_to_num(X_all).swapaxes(1, 2)  # 转置为 (n_samples, n_channels, n_timepoints)
    ...
    # ======================
        ...
        if task_type == 'classification':
            ...
            X_train = np.nan_to_num(X_train_temp).swapaxes(1, 2)
            ...
            if hasattr(model_instance, '_model_params'):
                ...
                model = AEDCNNClusterer(**base_params)
            else:
                model = AEDCNNClusterer(
...
def _evaluate_on_test(model, env, X_test_temp, y_test_temp):
    ...
    try:
        ...
        if env.task_type == 'classification':
            # (N, C, T)
            X_test = np.nan_to_num(X_test_temp).swapaxes(1, 2)
...
class RLCleanEnvironment:
    ...
    def step_low(self, macro_action, low_action):
        ...
        R_Structure = MU_1 * (after_structure - before_structure)
        R_Conservative = -MU_2 * np.sum((self.cur_train[:, :, self.numeric_cols] - before_cols) ** 2)
        R_Local_Signal = MU_3 * (initial_rate - final_rate)
        R_L = R_Structure + R_Conservative + R_Local_Signal
    ...
    def step_high(self, macro_action, low_reward, is_final=False):
        ...
        if is_final or self.step_count > self.max_steps:
            ...
            R_H = LAMBDA_1 * (final_perf - self.initial_final_perf)
            # 提供更详细的下游任务性能信息
            info = {
        ...
        R_Gradient = LAMBDA_GRAD * test_perf_change + 5.0 * k_stability
        
        # 添加指标改进的奖励
        R_Issue = LAMBDA_3 * (missing_improvement + outlier_improvement + violation_improvement)
        
        R_Time = -EPSILON * 1.0
        time_inc = cur_time - self.proxy_train_time_last
        R_Cost = -LAMBDA_2 * max(0, time_inc)
        
        # 如果当前动作是重复且无效的，添加额外的负奖励
        extra_penalty = 0.0
        if len(self.action_history) >= 2:
            recent_actions = self.action_history[-2:]
            recent_performances = self.performance_history[-2:] if len(self.performance_history) >= 2 else []
            
            # 检查是否是重复动作且没有改善
            if (len(recent_actions) == 2 and recent_actions[0] == recent_actions[1] 
                and len(recent_performances) == 2 and recent_performances[0] <= 0 and recent_performances[1] <= 0):
                extra_penalty = -5.0  # 添加额外的负奖励
                
        R_H = R_Gradient + R_Issue + ALPHA_RL * low_reward + R_Time + R_Cost + extra_penalty
        ...
        info = {
            'grad_reward': R_Gradient,
            'issue_reward': R_Issue,
            'best_k': self.best_k,
...
class LowAgent(HRLAgent):
    ...
    def choose(self, state, macro_action):
        ...
        if macro_action not in self.q:
            return np.random.choice(low_actions)
        if key not in self.q[macro_action]:
            self.q[macro_action][key] = np.zeros(len(low_actions))
    ...
    def update(self, trajectory, macro_action):
        ...
        if k not in self.q[macro_action]:
            self.q[macro_action][k] = np.zeros(len(low_actions))
        next_max_q = 0.0
...
def macro_agent_choose_with_rates(macro_agent, state, current_issue_rates, env=None):
    ...
    if macro_agent.steps < 8:  # 前8步更倾向于随机探索
        ...
        macro_action = np.random.choice(explorable)
        return macro_action
    ...
    if env:
        ...
        if has_repeated:
            ...
            if explorable:
                ...
                return np.random.choice(explorable)

    if np.random.rand() < macro_agent.epsilon():
...
def low_agent_choose_override(low_agent, state, macro_action, env=None):
    ...
    if env:
        has_repeated, repeated_action = _has_consecutive_repeated_actions_with_minimal_gain(env)
        if has_repeated and repeated_action[0] == macro_action:
            # 强制选择其他low action
            current_low_action = repeated_action[1]
            explorable = [a for a in low_actions if a != current_low_action]
            if explorable:
                print(f"强制更换低级工具: 连续执行 {repeated_action} 两次且无性能提升，强制选择其他工具")
                return np.random.choice(explorable)
    ...
    if np.random.rand() < low_agent.epsilon():
        return np.random.choice(low_actions)
    ...
    return low_actions[np.argmax(q_values)]
...
if __name__ == '__main__':
    ...
    task_type = 'forecast'
    model_save_path = f"/home/syh/TSC/AutoClean/Error_Cleaner/saved_models/original/{task_type}_{dataset_name}_original_5.pkl"
